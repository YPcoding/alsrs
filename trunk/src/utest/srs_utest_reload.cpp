...
VOID TEST(ConfigReloadTest, ReloadHttpStreamUpdated)
{
    ...
    conf.subscribe(&handler);
    EXPECT_TRUE(ERROR_SUCCESS == conf.parse(_MIN_OK_CONF"http_stream {enabled on; listen 5678;}"));
    EXPECT_TRUE(ERROR_SUCCESS == conf.do_reload(_MIN_OK_CONF"http_stream {enabled on; listen 5678;}"));
    EXPECT_TRUE(handler.all_false());
    handler.reset();
